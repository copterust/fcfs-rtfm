...
pub fn body_rate(state: &types::State, control: &types::Control) -> ([f32; 3],
                                                                     [f32; 3]) {
    ...
    let x_corr = x_err * control.pk + i_comp * control.ik + control.dk * delta_x;
    let y_corr = y_err * control.pk + i_comp * control.ik + control.dk * delta_y;
